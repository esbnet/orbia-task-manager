import { UpdateDailySubtaskUseCase } from "@/application/use-cases/use-cases/daily-subtask/update-daily-subtask/update-daily-subtask-use-case";
import { InMemoryDailyRepository } from "@/infra/repositories/memory/in-memory-daily-repository";

describe("UpdateDailySubtaskUseCase", () => {
	let useCase: UpdateDailySubtaskUseCase;
	let dailyRepository: InMemoryDailyRepository;

	beforeEach(() => {
		dailyRepository = new InMemoryDailyRepository();
		useCase = new UpdateDailySubtaskUseCase(dailyRepository);
	});
	...
	it("deve executar o caso de uso com sucesso", async () => {
		// TODO: Implementar teste específico para UpdateDailySubtaskUseCase
