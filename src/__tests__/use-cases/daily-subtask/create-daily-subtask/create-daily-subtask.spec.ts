import { CreateDailySubtaskUseCase } from "@/application/use-cases/use-cases/daily-subtask/create-daily-subtask/create-daily-subtask-use-case";
import { InMemoryDailyRepository } from "@/infra/repositories/memory/in-memory-daily-repository";

describe("CreateDailySubtaskUseCase", () => {
	let useCase: CreateDailySubtaskUseCase;
	let dailyRepository: InMemoryDailyRepository;

	beforeEach(() => {
		dailyRepository = new InMemoryDailyRepository();
		useCase = new CreateDailySubtaskUseCase(dailyRepository);
	});
	...
	it("deve executar o caso de uso com sucesso", async () => {
		// TODO: Implementar teste específico para CreateDailySubtaskUseCase
