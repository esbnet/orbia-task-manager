import { CompleteHabitWithLogUseCase } from "@/application/use-cases/use-cases/habit/complete-habit-with-log/complete-habit-with-log-use-case";
import { InMemoryDailyRepository } from "@/infra/repositories/memory/in-memory-daily-repository";

describe("CompleteHabitWithLogUseCase", () => {
	let useCase: CompleteHabitWithLogUseCase;
	let dailyRepository: InMemoryDailyRepository;

	beforeEach(() => {
		dailyRepository = new InMemoryDailyRepository();
		useCase = new CompleteHabitWithLogUseCase(dailyRepository);
	});

	afterEach(async () => {
		// Limpar todos os dailies criados durante o teste
		const allDailies = await dailyRepository.list();
		for (const daily of allDailies) {
			await dailyRepository.delete(daily.id);
		}
	});

	it("deve executar o caso de uso com sucesso", async () => {
		// TODO: Implementar teste específico para CompleteHabitWithLogUseCase
		const result = await useCase.execute();

		expect(result).toBeDefined();
	});
});
