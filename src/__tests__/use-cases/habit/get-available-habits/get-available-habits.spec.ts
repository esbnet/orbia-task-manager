import { GetAvailableHabitsUseCase } from "@/application/use-cases/use-cases/habit/get-available-habits/get-available-habits-use-case";
import { InMemoryDailyRepository } from "@/infra/repositories/memory/in-memory-daily-repository";

describe("GetAvailableHabitsUseCase", () => {
	let useCase: GetAvailableHabitsUseCase;
	let dailyRepository: InMemoryDailyRepository;

	beforeEach(() => {
		dailyRepository = new InMemoryDailyRepository();
		useCase = new GetAvailableHabitsUseCase(dailyRepository);
	});
	...
	it("deve executar o caso de uso com sucesso", async () => {
		// TODO: Implementar teste específico para GetAvailableHabitsUseCase
