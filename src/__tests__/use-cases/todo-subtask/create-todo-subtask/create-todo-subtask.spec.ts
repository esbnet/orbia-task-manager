import { CreateTodoSubtaskUseCase } from "@/application/use-cases/use-cases/todo-subtask/create-todo-subtask/create-todo-subtask-use-case";
import { InMemoryDailyRepository } from "@/infra/repositories/memory/in-memory-daily-repository";

describe("CreateTodoSubtaskUseCase", () => {
	let useCase: CreateTodoSubtaskUseCase;
	let dailyRepository: InMemoryDailyRepository;

	beforeEach(() => {
		dailyRepository = new InMemoryDailyRepository();
		useCase = new CreateTodoSubtaskUseCase(dailyRepository);
	});
	...
	it("deve executar o caso de uso com sucesso", async () => {
		// TODO: Implementar teste específico para CreateTodoSubtaskUseCase
